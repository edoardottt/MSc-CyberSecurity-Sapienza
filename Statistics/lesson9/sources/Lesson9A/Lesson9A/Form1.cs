...
namespace Lesson9A
{
    public partial class Form1 : Form
    {
        ...
        int deviation = 0; // deviation (user input)
        ...
        Dictionary<int,ArrayList> values;
        Bitmap b;
        Graphics g;
        Rectangle viewport;
        ...
        private bool checkInput()
        {
            ...
            deviation = 0;
            if (!Int32.TryParse(textBox3.Text, out deviation))
            {
                richTextBox1.Text = "deviation must be a valid integer!";
                return false;
            }
            return true;
        }
        ...
        {
            return autoRand.NextDouble() * (8.0 - (- 8.0)) + (- 8.0);
        }
        ...
        private void drawPaths()
        {
            ...
            double sqrt = Math.Sqrt((double)1/(double)n);
            for (int i = 0; i < m; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    double success = (value - 0) / (double)deviation;
                    successNow = successNow + (deviation * sqrt * success);
                    values[i].Add(successNow);
                    if (successNow < minY_Window) minY_Window = successNow;
                    if (successNow > maxY_Window) maxY_Window = successNow;
                }
            }
        ...
        // == PLOT PATHS ==
        private void plotPaths(Dictionary<int, ArrayList> values)
        {
            for (int i=0; i < m; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    if (j == 0)
                    {
                        ...
                        int Yviewport = calculateYViewport((double)values[i][j], viewport, minY_Window, rangeY);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            values = new Dictionary<int, ArrayList>();
