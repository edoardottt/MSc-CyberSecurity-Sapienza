...
We look into the SPD that points to certain SA entries in SA Database, then we can process the packet.  
...
In transport mode if we have to send a packet from A to B, we encrypt only the data plus some IPv6 extension headers following the ESP header, then we send it encrypted over the network until it reaches the desination host, then it can be decrypted.
...
3) Noone modified the packet 

AH prevents address spoofing and replay attacks.

The A.H. can be used in two ways:  

- Tunnel Mode (Gateway to Gateway) :

	In tunnel mode if we have to send a packet from A to B, we have to reach the first Gateway A with the unauthenticated data, then the Gateway A will authenticate all the data we send (so Header + Data) and then the Gateway will send it until it reaches the Gateway B.

- Transport Mode (Host to Host):

	In transport mode if we have to send a packet from A to B, we authenticate only the data plus some IPv6 extension headers following the AH header, then we send it over the network until it reaches the desination host.  
	The header fields not authenticated can change during the communication. 
...
In the traditional networks, the data and the control plane is overlapped into routers. The routers have the responsability to both forward the packets from an input port to an output port (forwarding, data plane) and decide what route the packets have to follow (routing, control plane).  
With this type of network there are some problems: every packets or connections have different objectives in respect to others, but they follow exactly the same routes.  
With a Software Defined Network we split the control and the data plane in two different parts, enhancing the network capabilities.  
We move the "smart" part of the network from the routers to a centralized part where all the control plane is done.  
Phisically we have a central operation center which controls all the routers and can decide the routing tables, rules, actions and so on.  
